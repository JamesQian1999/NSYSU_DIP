...
def open_file():
    try:
        global openfile
        openfile = "Lenna_512_color.tif"  # open the image file
        # Choose the file
...
def HSI(): # used the HSI formula
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if      den == 0:  H = 0
            elif    B[i, j] <= G[i, j]: H = theta
            else:   H = 2 * 3.14169265 - theta

            min_RGB = min(min(B[i, j], G[i, j]), R[i, j])
            sum = B[i, j] + G[i, j] + R[i, j]
            if sum == 0:    S = 0
            else:           S = 1 - 3 * min_RGB / sum
            H = H / (2 * 3.14159265)
            I = sum / 3.0
    ...
    forget_feather()
    return hsi # return the hsi image

# creat the HSI button
...
def Enhance():
    ...
    # used complements to enhance the detail
    enhance = 255 - enhance
...
# creat the "smoothing and sharping" button
...
def feather():
    ...
    # select the hue in range 120 to 220
    for i in range(rows):
        for j in range(cols):
            B,G,R=finalload[i,j]
            B=int(B)
    ...
    step1 = cv2.resize(finalload, (200, 200), interpolation = cv2.INTER_CUBIC)
    step1 = ImageTk.PhotoImage(image=Image.fromarray(cv2.cvtColor(step1, cv2.COLOR_BGR2RGB)))
    STEP1 = tk.Label(window, image=step1, width=200, height=200)
    STEP1.place(x=300, y=500)
    STEP1_label = tk.Label(window, text="Step 1",width=10, height=1)
    STEP1_label.place(x=300, y=710)

    
    # select the saturation in range 50 to 120
    for i in range(rows):
        for j in range(cols):
            B,G,R=finalload[i,j]
            B=int(B)
    ...
    step2 = cv2.resize(finalload, (200, 200), interpolation = cv2.INTER_CUBIC)
    step2 = ImageTk.PhotoImage(image=Image.fromarray(cv2.cvtColor(step2, cv2.COLOR_BGR2RGB)))
    STEP2 = tk.Label(window, image=step2, width=200, height=200)
    STEP2.place(x=500, y=500)
    STEP2_label = tk.Label(window, text="Step 2",width=10, height=1)
    STEP2_label.place(x=500, y=710)

    # select the saturation in range 50 to 120 and hue in range 120 to 220
    for i in range(rows):
        for j in range(cols):
            B,G,R=finalload[i,j]
            B=int(B)
    ...
    step3 = cv2.resize(finalload, (200, 200), interpolation = cv2.INTER_CUBIC)
    step3 = ImageTk.PhotoImage(image=Image.fromarray(cv2.cvtColor(step3, cv2.COLOR_BGR2RGB)))
    STEP3 = tk.Label(window, image=step3, width=200, height=200)
    STEP3.place(x=700, y=500)
    STEP3_label = tk.Label(window, text="Step 3",width=10, height=1)
    STEP3_label.place(x=700, y=710)
